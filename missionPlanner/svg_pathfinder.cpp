...
int scaleToGrid(double coord, double svgMax) {
  return std::round(coord / (svgMax) * (GRID_SIZE - 1));
}
bool pointInCircle(int x, int y, double cx, double cy, double r) {
  double dx = x - cx, dy = y - cy;
  return dx * dx + dy * dy <= r * r;
}
bool pointOnLine(int x, int y, double x1, double y1, double x2, double y2,
                 double thickness) {
...
std::vector<Node> astar(Cell grid[GRID_SIZE][GRID_SIZE], int sx, int sy, int ex,
                        int ey) {
  ...
  while (!open.empty()) {
    ...
    for (int dir = 0; dir < 8; dir++) {
      ...
      if (nx < 0 || ny < 0 || nx >= GRID_SIZE || ny >= GRID_SIZE)
        continue;
      if (grid[nx][ny].blocked || closed[nx][ny])
        continue;
...
int main() {
  ...
  if (doc.LoadFile("map.svg") != XML_SUCCESS) {
    std::cout << "Failed to load SVG\n";
    return 1;
  }
  XMLElement *svg = doc.FirstChildElement("svg");
  if (!svg) {
    std::cout << "No <svg> element found\n";
    return 1;
  }
  ...
  for (XMLElement *path =
           svg->FirstChildElement("g")->FirstChildElement("path");
  ...
  for (XMLElement *circ =
           svg->FirstChildElement("g")->FirstChildElement("circle");
       circ; circ = circ->NextSiblingElement("circle")) {
    double cx = circ->DoubleAttribute("cx"), cy = circ->DoubleAttribute("cy"),
           r = circ->DoubleAttribute("r");
    const char *id = circ->Attribute("id");
    if (id && std::string(id) == "A") {
      ax = cx;
      ay = cy;
      ar = r;
    } else if (id && std::string(id) == "B") {
      bx = cx;
      by = cy;
      br = r;
    }
  ...
  for (XMLElement *ell =
           svg->FirstChildElement("g")->FirstChildElement("ellipse");
       ell; ell = ell->NextSiblingElement("ellipse")) {
  ...
  for (int i = 0; i < GRID_SIZE; i++) {
    for (int j = 0; j < GRID_SIZE; j++) {
      ...
      for (auto &w : walls) {
        double x1, y1, x2, y2;
        std::tie(x1, y1, x2, y2) = w;
        if (pointOnLine(x, y, x1, y1, x2, y2, thickness)) {
          grid[i][j].blocked = true;
          break;
        }
      }
      if (pointInCircle(x, y, ax, ay, ar))
        grid[i][j].isStart = true;
      if (pointInCircle(x, y, bx, by, br))
        grid[i][j].isEnd = true;
    }
  }
  ...
  if (sx == -1 || ex == -1) {
    std::cout << "Start or End not found!\n";
    return 1;
  }
 // print grid
  for (int j = 0; j < GRID_SIZE; j++) {
  ...
  out << "<svg width=\"200cm\" height=\"200cm\" viewBox=\"0 0 4000 4000\" "
  ...
  for (auto &w : walls) {
    ...
    out << "<line x1=\"" << x1 << "\" y1=\"" << y1 << "\" x2=\"" << x2
        << "\" y2=\"" << y2 << "\" stroke=\"red\" stroke-width=\"20\" />\n";
  }

  // Original circles
  out << "<circle cx=\"" << ax << "\" cy=\"" << ay << "\" r=\"" << ar
      << "\" fill=\"red\" />\n";
  out << "<ellipse cx=\"" << bx << "\" cy=\"" << by << "\" rx=\"" << br
      << "\" ry=\"" << br << "\" fill=\"red\" />\n";

  // Draw path
  out << "<polyline fill=\"none\" stroke=\"blue\" stroke-width=\"15\" "
         "points=\"";
  for (auto &n : path) {
